...
mkdir -p "$DEST"/usr/lib/systemd/system/local-fs.target.requires

cat > "$DEST"/usr/lib/systemd/system/usr.mount <<EOF
# Automatically generated by systemd-btrfs-usr-generator
...
Where=/usr
Type=btrfs
Options=subvol=$usrsubvol
EOF

ln -snfr "$DEST"/usr/lib/systemd/system/usr.mount "$DEST"/usr/lib/systemd/system/local-fs.target.requires/usr.mount
...
if [[ $vol != "usr" ]]; then
    ...
    xz -T0 $TMPSTORE/usr:$OS_ARCH:"${vol##*:}-$VERSION".btrfsinc
